...
const PLATFORMS_STATUS = [
  { name:'instagram' as PlatKey, connected:true,  followers:'12.4K', growth:'+3.2%' },
  { name:'youtube'   as PlatKey, connected:true,  followers:'8.1K',  growth:'+5.8%' },
  { name:'twitter'   as PlatKey, connected:false, followers:'—',     growth:'—'     },
]
...
export function DashboardSection() {
  ...
  // Weekly posts (last 7 days bucketed by day)
  const weeklyBuckets = (() => {
  ...
  // Activity feed derived from real posts
  ...
  const KPI = [
    ...
    { label:'Published',       value: loading ? '—' : published.length.toString(),  sub:'Live content',       color:C.accent2, pct: totalPosts > 0 ? (published.length / totalPosts) * 100 : 0 },
    { label:'Scheduled',       value: loading ? '—' : scheduled.length.toString(),  sub:'Queued to publish',  color:C.accent3, pct: totalPosts > 0 ? (scheduled.length / totalPosts) * 100 : 0 },
    { label:'Drafts',          value: loading ? '—' : drafts.length.toString(),     sub:'Work in progress',   color:C.accent4, pct: totalPosts > 0 ? (drafts.length / totalPosts) * 100 : 0 },
  ]
